...
**Фаза 6 — добивка. Сложность: средняя. ✅ БЭКЕНД РЕАЛИЗОВАН** *(без фронтенд-интеграции и прод-деплоя; runtime-прогоны e2e/k6 требуют запущенного Docker-стека и установленного k6)*
- [x] `scheduleApi` в `company`: порт `schedule.ts` с тестами, upsert шаблона `saveSchedule`, транзакционный батч-upsert исключений, gRPC и gateway REST.
- [ ] Переключение модуля графика во фронтенде. *(исключено из текущей задачи по уточнению пользователя; фронтенд-репозиторий не изменялся)*
- [x] `distributionApi` в `company`: схема §8.1.1, порт `dealDistribution.ts` с тестами, CRUD и транзакционный выбор исполнителя под `SELECT FOR UPDATE`.
- [ ] Переключение модуля распределения (`VITE_API_MODE_DISTRIBUTION`) во фронтенде. *(исключено из текущей задачи по уточнению пользователя)*
- [ ] Убрать `networkMode: 'always'` во фронтенде. *(исключено из текущей задачи по уточнению пользователя)*
- [x] E2E-смоук §17 добавлен в `tests/e2e/smoke.sh`, включая SSE и обновление link-урока. *(синтаксис проверен; runtime-прогон не выполнен, потому что Docker daemon остановлен)*
- [x] Сервис `files` + MinIO/S3: отдельная БД, потоковая загрузка вложений/аватаров/логотипов, MIME/size-проверки, presigned URL, gRPC, gateway REST, миграция и Compose.
- [x] Observability-профиль: Prometheus/Grafana/Loki/Tempo/OTel Collector, RED/outbox/consumer-lag/DLQ-метрики, алерты и дашборд.
- [x] k6-профили добавлены для чтения БЗ/задач и конкурентного `moveTask`. *(синтаксис проверен; нагрузочный runtime-прогон не выполнен, потому что k6 не установлен и Docker daemon остановлен)*
- [ ] Прод-деплой на сервер по-настоящему, без Docker (§15.1): Go-бинарники под systemd, штатные Postgres и NATS, бэкапы (wal-g), runbook с рецептами §3.4–3.5. *(исключён из текущей задачи)*
