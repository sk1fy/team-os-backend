...
### 9.1 Текущая модель авторизации файлов

`GET /api/v1/files/{id}` проверяет `company_id`, но пока не проверяет доступ к задаче или
другой родительской сущности. Любой авторизованный пользователь компании, получивший UUID
файла, может получить короткоживущую presigned-ссылку. UUID не считается границей авторизации; это
известное ограничение текущего контракта.

`DELETE /api/v1/files/{id}` разрешён только загрузившему файл, `owner` или `admin`.

До внедрения ACL нельзя публиковать UUID файла вне той сущности, к которой у пользователя есть
доступ. Целевое решение: аддитивно добавить к метаданным файла тип и UUID родительской сущности,
проверять её доменные права перед выдачей ссылки и добавить negative integration tests для недоступных
задач и других родителей.
