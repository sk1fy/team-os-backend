...
Шесть доменных сервисов + gateway (BFF). Каждый сервис владеет своей базой и является самостоятельным
деплой-юнитом.

| Сервис | Порт | Владеет |
|---|---|---|
| `gateway` | 8080 | маршрутизация, проверка JWT, CORS, rate limiting, SSE-прокси (данными не владеет) |
| `company` | 8081 | авторизация, пользователи, оргструктура, графики, распределение сделок, синхронизация amoCRM |
| `kb` | 8082 | база знаний: секции, статьи, версии, ознакомления, наследуемые доступы |
| `tasks` | 8083 | таск-трекер: доски, колонки, задачи, комментарии, метки |
| `academy` | 8084 | курсы, уроки, квизы, назначения, прогресс |
| `notifications` | 8085 | уведомления + SSE |
| `files` | 8086 | загрузка файлов, presigned-ссылки (MinIO/S3) |
...
pkg/               # общие библиотеки (отдельный модуль): apierror, auth, eventbus, httpx, richtext
services/          # по одному Go-модулю на сервис (company, gateway, ...)
tools/             # вспомогательные инструменты (отдельный модуль): sync-contract
deploy/            # docker-compose.yaml (+ docker-compose.prod.yaml), инициализация postgres
...
- `make check-contract` — линт OpenAPI + buf, затем `tools/sync-contract` сверяет вызовы
  `src/api` фронтенда (`FRONTEND_DIR`) с путями и методами `teamos.yaml`.
